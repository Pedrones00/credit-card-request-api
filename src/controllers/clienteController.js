...
class ClienteController {
  constructor(ClienteModel, CartaoModel, ContratoModel) {
    ...
    this.mutableRequestFields = {
      fields: ["nome", "cpf", "email", "dt_nascimento", "cpf_regular"],
    };
  }
  ...
  #validateRegisterInputs(request) {
    ...
    if (!request.body)
      this.#throwError(
        400,
        "Requisição sem corpo, é necessário o envio de informações para o cadastro de um cliente"
      );

    if (!request.body.nome) errorMessages.push("nome: nome do cliente");
    if (!request.body.cpf) errorMessages.push("cpf: cpf do cliente");
    if (!request.body.dt_nascimento)
      errorMessages.push("dt_nascimento: data de nascimento do cliente");

    if (errorMessages.length)
      this.#throwError(
        400,
        `Campos obrigatórios não preenchidos: ${errorMessages}`
      );
  }
  ...
  #validateUpdateInputs(request) {
    ...
    if (!request.body.id_cliente)
      this.#throwError(400, "O ID do cliente está ausente (id_cliente)");
  }
  ...
  async #validateDuplicateCPF(cpf_cliente) {
    ...
    if (cpfInUse)
      this.#throwError(
        409,
        `CPF já existe na base para o cliente id ${cpfInUse.id_cliente}`
      );
  }
  ...
  async #listAll(
    ...
    const clientes = await this.Cliente.findAll({
      ...
      include: this.#createSequelizeIncludeArrays(
        detailsContrato,
        detailsCartao
      ),
    });
  ...
  async #register(request) {
    ...
    const newCliente = await this.Cliente.create(
      request.body,
      this.mutableRequestFields
    );
  ...
  async #updateCliente(request) {
    ...
    if (request.body.cpf && request.body.cpf !== cliente.cpf)
      await this.#validateDuplicateCPF(request.body.cpf);
  ...
  async #deleteCliente(id_cliente) {
    const cliente = await this.Cliente.findByPk(id_cliente);
    if (!cliente) this.#throwError(404, "Cliente não encontrado");
    if (cliente.cliente_ativo === false)
      this.#throwError(
        400,
        "O cliente já está desativado, nenhum modificação foi realizada"
      );

    await cliente.update({ cliente_ativo: false });

    const contratos_desativados = await this.#disableContratos(
      cliente.id_cliente
    );
  ...
  async #activateCliente(id_cliente) {
    ...
    if (cliente.cliente_ativo === true)
      this.#throwError(
        400,
        "O cliente já está ativo, nenhum modificação foi realizada"
      );
  ...
  async listAllAPI(request, response) {
    try {
      ...
      const arrayQueryDetails = Array.isArray(request.query.details)
      ...
      const clientes = await this.#listAll(
        clienteState,
        cpfCliente,
        detailsContrato,
        detailsCartao
  ...
  async searchIDAPI(request, response) {
    try {
      ...
      const cliente = await this.#searchID(
        id_cliente,
        detailsContrato,
        detailsCartao
      );
  ...
  async viewPage(request, response) {
    try {
      ...
      const cliente = await this.#searchID(id, false, true);
